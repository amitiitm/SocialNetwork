  ...
  module ByPassword
    ...
    def self.included(recipient)
      ...
      recipient.class_eval do
        ...
        validates_presence_of     :password_confirmation,      :if => :password_required?, :message => "Password confirmation can't be blank"
        validates_confirmation_of :password,                   :if => :password_required?, :message => "Password confirmation does not match"
        validates_length_of       :password, :within => 5..40, :if => :password_blank?, :message => "Password should be atleast 5 characters"
