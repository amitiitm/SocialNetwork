...
describe SessionsController do
  ...
  describe "logout_killing_session!" do
    ...
    it 'nils the current admin_user'      do logout_killing_session!; current_admin_user.should be_nil end
    it 'kills :admin_user_id session' do
      session.stub!(:[]=)
      session.should_receive(:[]=).with(:admin_user_id, nil).at_least(:once)
      logout_killing_session!
    end
    it 'forgets me' do    
      current_admin_user.remember_me
      current_admin_user.remember_token.should_not be_nil; current_admin_user.remember_token_expires_at.should_not be_nil
      AdminUser.find(1).remember_token.should_not be_nil; AdminUser.find(1).remember_token_expires_at.should_not be_nil
      logout_killing_session!
      AdminUser.find(1).remember_token.should     be_nil; AdminUser.find(1).remember_token_expires_at.should     be_nil
    end
  end
  ...
  describe "logout_keeping_session!" do
    ...
    it 'nils the current admin_user'      do logout_keeping_session!; current_admin_user.should be_nil end
    it 'kills :admin_user_id session' do
      session.stub!(:[]=)
      session.should_receive(:[]=).with(:admin_user_id, nil).at_least(:once)
  ...
  describe "Logging in by cookie" do
    def set_remember_token token, time
      @admin_user[:remember_token]            = token; 
      @admin_user[:remember_token_expires_at] = time
      @admin_user.save!
    end    
    before do 
      @admin_user = AdminUser.find(:first); 
