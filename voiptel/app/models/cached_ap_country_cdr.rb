class CachedApCountryCdr < ActiveRecord::Base
  def self.update(options)
    ...
    cache = 
      self.find(:first, :conditions => {:month => month, :country_id => cdr.country_id}) ||
      self.new(:month => month, :country_id => cdr.country_id)
    cache.calls += 1
