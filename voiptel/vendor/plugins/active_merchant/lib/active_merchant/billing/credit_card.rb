  ...
  module Billing #:nodoc:
    ...
    class CreditCard
      ...
      def validate_card_number #:nodoc:
        errors.add :number, "Card number is not a valid credit card number" unless CreditCard.valid_number?(number)
        unless errors.on(:number) || errors.on(:type)
          errors.add :type, "Cart type is not the correct card type" unless CreditCard.matching_type?(number, type)
        end
      end
      ...
      def validate_essential_attributes #:nodoc:
        errors.add :first_name, "First name cannot be empty"      if @first_name.blank?
        errors.add :last_name,  "Last name cannot be empty"      if @last_name.blank?
        errors.add :month,      "Expiration month is not a valid month" unless valid_month?(@month)
        errors.add :year,       ""              if expired?
        errors.add :year,       "Expiration year is not a valid year"  unless valid_expiry_year?(@year)
      end
      ...
      def validate_verification_value #:nodoc:
        if CreditCard.requires_verification_value?
          errors.add :verification_value, "Security Code is required" unless verification_value? 
        end
      end
    end
  end
