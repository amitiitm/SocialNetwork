class Production::ProductionPadCrud
  include General
  ...
  #  V	Virtual Order
  #  F	Spec Order
  def self.pad_film_inbox(doc) ### not in use
    catalog_attribute_code = @config["catalog_attribute_code"]
    attribute_value_pad = @config["attribute_value_pad"]
    workflow_pad = @config["workflow_pad"]
    ...
    condition = "(so.company_id = #{criteria.company_id}) AND
                     ...
                     AND sol.blank_good_flag <> 'Y' AND sol.item_type = 'I' and sol.trans_flag = 'A' and sol.film_flag = 'N'
                     AND (soav.catalog_attribute_code = '#{catalog_attribute_code}' and soav.catalog_attribute_value_code = '#{attribute_value_pad}' and catalog_items.workflow = '#{workflow_pad}')
    ...
    Sales::SalesOrder.find_by_sql ["select CAST(( select (
                                  select * from (select distinct '' as vs, '' as user_name,soa.artwork_version,so.ext_ref_no,types.description as order_type,so.trans_type,so.trans_date,customers.name,customers.code as customer_code,
                                         so.payment_status,so.rushorder_flag,so.artwork_status,so.order_status,so.trans_no,sol.serial_no,so.logo_name,sol.catalog_item_code,so.id,
...
UNION
  ...
  def self.pad_print_inbox(doc) ## not in use
    catalog_attribute_code = @config["catalog_attribute_code"]
    attribute_value_pad = @config["attribute_value_pad"]
    workflow_pad = @config["workflow_pad"]
    criteria = Setup::Criteria.fill_criteria_for_query(doc/:criteria)
...
UNION
                                  select distinct '' as vs, '' as user_name,soa.artwork_version,so.ext_ref_no,types.description as order_type,so.trans_type,so.trans_date,customers.name,customers.code as customer_code,
                                         so.payment_status,so.rushorder_flag,so.artwork_status,so.order_status,so.trans_no,sol.serial_no,so.logo_name,sol.catalog_item_code,sol.id,
                                         soa.artwork_file_name,sales_order_shippings.serial_no as shipping_serial_no,sol.indigo_code,sol.reason,sol.item_description,sales_order_shippings.ship_qty as item_qty,so.ship_date,so.order_flagged from sales_orders so
                                  inner join sales_order_lines sol on sol.sales_order_id = so.id
                                  inner join catalog_items on catalog_items.id = sol.catalog_item_id
                                  inner join customers on customers.id = so.customer_id
                                  inner join users on users.id = so.updated_by
