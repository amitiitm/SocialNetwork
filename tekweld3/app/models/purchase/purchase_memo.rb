class Purchase::PurchaseMemo < ActiveRecord::Base
  include UserStamp
  include Dbobject
  include General
  ...
  has_many :purchase_memo_lines, :class_name => 'Purchase::PurchaseMemoLine' , :extend=>ExtendAssosiation
  ...
  after_create do 
    Setup::Sequence.upd_book_code(self.trans_bk,'PAOIMO',self.trans_no,self.company_id)  if self.trans_bk == 'PM01' 
  end  

  def add_line_details(line_doc)
    id =  parse_xml(line_doc/'id') if (line_doc/'id').first
    line = memolines(line_doc.name,id) || nil
  ...
  def memolines(name,id)
    purchase_memo_lines.find_or_build(id) if name == 'purchase_memo_line' 
  end

  def save_lines
    purchase_memo_lines.save_line 
  end

  def add_line_errors_to_header
    add_line_item_errors(purchase_memo_lines)
  end
  ...
  def apply_header_fields_to_lines()
    self.purchase_memo_lines.each{ |line|
      #      if line.new_record?
