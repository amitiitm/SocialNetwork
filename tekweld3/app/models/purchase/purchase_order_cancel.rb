class Purchase::PurchaseOrderCancel < ActiveRecord::Base
  include UserStamp
  include Dbobject
  include General
  ...
  has_many :purchase_order_cancel_lines, :class_name => 'Purchase::PurchaseOrderCancelLine' , :extend=>ExtendAssosiation
  ...
  after_create do 
    Setup::Sequence.upd_book_code(self.trans_bk,'PAOIOH',self.trans_no,self.company_id)  if self.trans_bk == 'PH01' 
  end  

  def add_line_details(line_doc)
    id =  parse_xml(line_doc/'id') if (line_doc/'id').first
    line = order_cancellines(line_doc.name,id) || nil
  ...
  def order_cancellines(name,id)
    purchase_order_cancel_lines.find_or_build(id) if name == 'purchase_order_cancel_line' 
  end

  def save_lines
    purchase_order_cancel_lines.save_line 
  end

  def add_line_errors_to_header
    add_line_item_errors(purchase_order_cancel_lines)
  end
  ...
  def apply_header_fields_to_lines()
    self.purchase_order_cancel_lines.each{ |line|
      #      if line.new_record?
