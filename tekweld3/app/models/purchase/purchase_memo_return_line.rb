class Purchase::PurchaseMemoReturnLine < ActiveRecord::Base
  include UserStamp
  include Dbobject
  include General
  include ClassMethods

  belongs_to :purchase_memo_return, :class_name => 'Purchase::PurchaseMemoReturn'
  ...
  def before_validation
    errors.add(:clear_qty,"MemoReturn Quantity cannot be greater than memoed quantity") if clear_qty > item_qty
  end
  ...
  def add_line_details(line_doc)
    ...
    line.types = 'PurchaseMemoReturn'   if ( line and line.new_record?)
