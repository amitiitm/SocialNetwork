class Sales::SalesSampleOrderCrud
  include General
  ...
  ## Sample/Virtual Order services
  def self.list_sample_orders(doc)
  ...
  def self.show_sample_order(order_id)
    #    Sales::SalesOrder.find_all_by_id(order_id)
    Sales::SalesOrder.where(:id => order_id)
  end

  def self.create_sample_orders(doc,schema_name,url_with_port)
    orders = []
    (doc/:sales_orders/:sales_order).each{|order_doc|
      order = create_sample_order(order_doc,schema_name,url_with_port)
      orders <<  order if order
    }
    orders
  end

  def self.create_sample_order(doc,schema_name,url_with_port)
    order = add_or_modify_sample_order(doc,schema_name,url_with_port)
    return  if !order
  ...
  def self.add_or_modify_sample_order(doc,schema_name,url_with_port)
    ...
    order.run_block do
      ...
      build_lines(doc/:sales_order_shippings/:sales_order_shipping)
  ...
  private_class_method :create_sample_order, :add_or_modify_sample_order
end