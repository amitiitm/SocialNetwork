class Sales::SalesCreditInvoice < ActiveRecord::Base
  include UserStamp
  include Dbobject
  include General
  ...
  #  include Sales::SalesAccountsPosting
  # include Sales::SalesGlPosting

  attr_accessor  :max_serial_no
  
  has_many :sales_credit_invoice_lines, :class_name => 'Sales::SalesCreditInvoiceLine' , :extend=>ExtendAssosiation
  ...
  validates :trans_bk, :trans_no, :trans_date, :customer_id, :account_period_code, :presence => true
  ...
  after_create do 
    Setup::Sequence.upd_book_code(self.trans_bk,'SAOICR',self.trans_no,self.company_id)  if self.trans_bk == 'SC01' 
  end  
  ...
  def credit_invoicelines(name,id)
    sales_credit_invoice_lines.find_or_build(id) if name == 'sales_credit_invoice_line' 
  end

  def save_lines
    sales_credit_invoice_lines.save_line 
  end

  def add_line_errors_to_header
    add_line_item_errors(sales_credit_invoice_lines)
  end
  ...
  def apply_header_fields_to_lines()
    sales_credit_invoice_lines.each{ |line|
  ...
  def apply_line_fields_to_header() 
    total_qty = 0
    self.sales_credit_invoice_lines.each{ |line|
