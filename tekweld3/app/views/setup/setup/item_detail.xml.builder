...
xml.catalog_item{
  for catalog_item in @catalog_items
    ...
    xml.has_expired_date_flag(catalog_item.has_expired_date_flag)
    ...
    #    first_column_price = item_prices[0].column1 if item_prices[0]
    #    xml.blank_good_price(first_column_price - (first_column_price * 10)/100) if first_column_price
    ...
    xml.catalog_item_line{
      ...
      xml.accessory_items{
        #        accessory_items = Item::CatalogItemAccessory.find_by_sql("select * from catalog_item_accessories where
        #                     catalog_item_id = #{catalog_item.id}#") if catalog_item
        #        accessory_items.each{|accessory_item|
        #          if accessory_item.trans_flag == 'A'
  ...
  #these changes are done for LTM charges added
  xml.ltm_charges{
    ltm_charges = Item::CatalogItem.find_by_sql ["select * from catalog_items where store_code = 'LTM-CHARGE'"]
    for ltm_charge in ltm_charges
      xml.sales_order_lines{
        xml.sales_order_line{
          xml.item_description(ltm_charge.sale_description)
          xml.image_thumnail(ltm_charge.image_thumnail)
          xml.item_type(ltm_charge.item_type)
          xml.line_type("S")
          xml.unit(ltm_charge.unit)
          xml.cost(ltm_charge.cost)
          xml.name(ltm_charge.name)
          xml.catalog_item_code(ltm_charge.store_code)
          xml.setup_item_id(ltm_charge.id)
          xml.catalog_item_id(ltm_charge.id)
          xml.scope_flag(ltm_charge.scope_flag)
          xml.workflow(ltm_charge.workflow)
          xml.id('')
          for catalog_item_price in ltm_charge.catalog_item_prices
            if (catalog_item_price.trans_flag == 'A' and (catalog_item_price.from_date.to_date <= Time.now.to_date) and (Time.now.to_date <= catalog_item_price.to_date.to_date))
              xml.item_price(catalog_item_price.column1)
              xml.item_qty(1)
              xml.item_amt(1*(catalog_item_price.column1))
