...
xml.thirdpartyshippingcharge{
  for catalog_item in @default_shipping_items
    xml.sales_order_line{
      ...
      if @customer_shipping_prices
        xml.item_price(@customer_shipping_prices.column1)
        xml.item_qty(1)
        xml.item_amt(1*(@customer_shipping_prices.column1))
      else
