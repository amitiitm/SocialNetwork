...
xml.sales_order_lines{
  xml.old_option_sales_order_line{
    for catalog_item in @catalog_items
      xml.sales_order_line{
        ...
        if @cust_prices
          xml.item_price(@cust_prices.column1)
          xml.item_qty(1)
          xml.item_amt(1*(@cust_prices.column1))
        else
  ...
  xml.new_option_sales_order_line{
    for new_catalog_item in @new_catalog_items
      xml.sales_order_line{
        xml.item_description(new_catalog_item.sale_description)
        xml.image_thumnail(new_catalog_item.image_thumnail)
        xml.item_type(new_catalog_item.item_type)
        xml.line_type("S")
        xml.unit(new_catalog_item.unit)
        xml.cost(new_catalog_item.cost)
        xml.name(new_catalog_item.name)
        xml.catalog_item_code(new_catalog_item.store_code)
        xml.setup_item_id(new_catalog_item.id)
        xml.catalog_item_id(new_catalog_item.id)
        xml.scope_flag(new_catalog_item.scope_flag)
        xml.workflow(new_catalog_item.workflow)
        xml.id('')
        ## to define customer specific price in between date range and charge code
        if @new_cust_prices
          xml.item_price(@new_cust_prices.column1)
          xml.item_qty(1)
          xml.item_amt(1*(@new_cust_prices.column1))
        else
          for catalog_item_price in new_catalog_item.catalog_item_prices
            if (catalog_item_price.trans_flag == 'A' and (catalog_item_price.from_date.to_date <= Time.now.to_date) and (Time.now.to_date <= catalog_item_price.to_date.to_date))
              xml.item_price(catalog_item_price.column1)
              xml.item_qty(1)
              xml.item_amt(1*(catalog_item_price.column1))
