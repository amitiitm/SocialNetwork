...
xml.purchase_order_cancels{
  for purchase_order_cancel in @order_cancels
    xml.purchase_order_cancel do
      ...
      xml.purchase_order_cancel_lines{
        for purchase_order_cancel_line in purchase_order_cancel.purchase_order_cancel_lines
          if purchase_order_cancel_line.trans_flag == 'A'
            xml.purchase_order_cancel_line do
              purchase_order_cancel_line.attributes.each_pair{ |column_name,column_value|
                column_value = format_column(column_value)
                xml.tag!(column_name, column_value)
              }
              for bom in TransactionBom::TransactionBom.find_by_sql ["select * from transaction_boms where trans_flag='A' and  id=?",purchase_order_cancel_line.transaction_bom_id]
                bom.attributes.each_pair{ |column_name,column_value|
                  column_value = format_column(column_value)
                  xml.tag!(column_name, column_value) unless  fields.include?(column_name)
                }
              end
              if purchase_order_cancel_line.catalog_item
                xml.taxable(purchase_order_cancel_line.catalog_item.taxable)
                xml.packet_require_yn(purchase_order_cancel_line.catalog_item.packet_require_yn)
                xml.image_thumnail(purchase_order_cancel_line.catalog_item.image_thumnail) if  ( !bom_yn || bom_yn.value=='N')
