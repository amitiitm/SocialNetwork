        ...
        for purchase_memo_return_line in purchase_memo_return.purchase_memo_return_lines
          if purchase_memo_return_line.trans_flag == 'A'
            xml.purchase_memo_return_line do
              purchase_memo_return_line.attributes.each_pair{ |column_name,column_value|
                column_value = format_column(column_value)
                xml.tag!(column_name, column_value)
              }
              for bom in TransactionBom::TransactionBom.find_by_sql ["select * from transaction_boms where trans_flag='A' and  id=?",purchase_memo_return_line.transaction_bom_id]
                bom.attributes.each_pair{ |column_name,column_value|
                  column_value = format_column(column_value)
                  xml.tag!(column_name, column_value) unless  fields.include?(column_name)
                }
              end
              if purchase_memo_return_line.catalog_item
                ...
                xml.image_thumnail(purchase_memo_return_line.catalog_item.image_thumnail) if  ( !bom_yn || bom_yn.value=='N')
