...
xml.tag!("#{@tag_level1}"){
  ...
  @data_rows.each_with_index {|row,index|
    ...
    xml.tag!("#{@tag_level2}") do
      row.each_with_index{|column,index|
        ...
        when 0
          if !(column.length==0)
            ...
            invoice = Customer::CustomerInvoice.find_by_trans_no(column)
            if invoice
        ...
        when 2
          if column
            ...
            if !customer
              msg = msg+"Customer# does not exist.please enter valid customer# \\n "
            else
              customer.attributes.each_pair{ |column_name,column_value|
                column_value = format_column(column_value)
                if column_name == 'id'
                  xml.customer_id(column_value)
                else
            ...
            msg=msg+"Customer# can't be Blank \\n"
          end 
        when 3
          if (column and column!='')
            column=column.to_i.to_s if !(col_len < column.length) and !(column.length==0)
            inv_type = Setup::Type.find_by_type_cd_and_subtype_cd_and_value( 'faar','inv_type',column)
            if !inv_type
      ...
      if gl_account
        xml.customer_invoice_line_gl_account_id(gl_account.id)
        xml.customer_invoice_line_gl_code(gl_account.code)
        xml.customer_invoice_line_gl_name(gl_account.name)
      end
    end if index > 0
  }
}