class Purchase::PurchaseInvoiceController < ApplicationController
  require 'hpricot'
  include General
  include ClassMethods
  
  #Invoice Services
  def list_invoices
    doc = Hpricot::XML(request.raw_post)
    doc = doc.to_s.gsub("&amp;","&") 
    doc = Hpricot::XML(doc)    
    @invoices = Purchase::PurchaseInvoiceCrud.list_invoices(doc)
    render :xml=>'<encoded>'+Base64.encode64(Zlib::Deflate.deflate(@invoices[0].xmlcol))+'</encoded>'
  end  

  def list_open_invoices
    doc = Hpricot::XML(request.raw_post)   
    doc = doc.to_s.gsub("&amp;","&") 
    doc = Hpricot::XML(doc)
    @invoices = Purchase::PurchaseInvoiceCrud.list_open_invoices(doc)
    render :xml=>@invoices[0].xmlcol
  end  

  def list_open_invoices_hdr
    doc = Hpricot::XML(request.raw_post)   
    doc = doc.to_s.gsub("&amp;","&") 
    doc = Hpricot::XML(doc)
    @invoices = Purchase::PurchaseInvoiceCrud.list_open_invoices_hdr(doc)
    render :xml=>@invoices[0].xmlcol
  end  

  def list_open_invoice_lines
    doc = Hpricot::XML(request.raw_post)   
    doc = doc.to_s.gsub("&amp;","&") 
    doc = Hpricot::XML(doc)
    @invoices = Purchase::PurchaseInvoiceCrud.list_open_invoice_lines(doc)
    render :xml=>@invoices[0].xmlcol
  end  

  def show_invoice
    doc = Hpricot::XML(request.raw_post)
    invoice_id  = parse_xml(doc/:params/'id')
    @login_type = parse_xml(doc/:params/'loginType')
    @invoices = Purchase::PurchaseInvoiceCrud.show_invoice(invoice_id)
    respond_to do |wants|
      wants.xml   
    end
  end  

  def create_invoices
    doc = Hpricot::XML(request.raw_post)   
    doc = doc.to_s.gsub("&amp;","&") 
    doc = Hpricot::XML(doc)
    @invoices= Purchase::PurchaseInvoiceCrud.create_invoices(doc)
    invoice =  @invoices.first if !@invoices.empty?
    if invoice.errors.empty?
      respond_to_action('show_invoice')
    else
      respond_to_errors(invoice.errors)
    end
  end

  #CreditInvoice Services
  def list_credit_invoices
    doc = Hpricot::XML(request.raw_post)   
    doc = doc.to_s.gsub("&amp;","&") 
    doc = Hpricot::XML(doc)
    @creditinvoices = Purchase::PurchaseCreditInvoiceCrud.list_credit_invoices(doc)
    render :xml=>'<encoded>'+Base64.encode64(Zlib::Deflate.deflate(@creditinvoices[0].xmlcol))+'</encoded>'
  end  

  def show_credit_invoice
    doc = Hpricot::XML(request.raw_post)
    creditinvoice_id  = parse_xml(doc/:params/'id')
    @creditinvoices = Purchase::PurchaseCreditInvoiceCrud.show_credit_invoice(creditinvoice_id)
    respond_to do |wants|
      wants.xml   
    end
  end  

  def create_credit_invoices
    doc = Hpricot::XML(request.raw_post)   
    doc = doc.to_s.gsub("&amp;","&") 
    doc = Hpricot::XML(doc)
    @creditinvoices= Purchase::PurchaseCreditInvoiceCrud.create_credit_invoices(doc)
    creditinvoice =  @creditinvoices.first if !@creditinvoices.empty?
    if creditinvoice.errors.empty?
      respond_to_action('show_credit_invoice')
    else
      respond_to_errors(creditinvoice.errors)
    end
  end
end
