...
private function setFetchParameter(invoice_type:String, fetch_type:String):void
{
	if(invoice_type == 'I')
	{
		if(fetch_type.toLocaleUpperCase() == 'GRID')
		{
			...
			dtlLine.title = "Open Invoices (Lines)"
...
private function createMapping():void
{
	...
	mappingArrCol.addItem({source: 'diamond_purchase_invoice_id', 	target: 'diamond_purchase_id', 	isPrimaryKey:'N',	updatable:'N'})
...
{
	if(dcTerm_code.text != '' && dfTrans_date.text != '')
	{
...
private function getPacketDetails():void
{
	...
	if(diamond_packet_no != '')
	{
		...
		getInformationEvent	= new GetInformationEvent('diamondpacketinfovalidate', callbacks, diamond_packet_no, diamond_lot_no, 'Y');
		getInformationEvent.dispatch();
	}
	else
	{
