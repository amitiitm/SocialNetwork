...
{
	var diam_loss_per:Number 	= 	__localModel.diamond_loss_percentage;

	var _stone_loss_amt:Number	=	Number(tiStone_loss_amt.dataValue);
	...
	if(diam_loss_per > 0)
	{
		_stone_loss_amt = ((_total_wt * _cost) * diam_loss_per)/ 100;
...
private function calculateAvgWt():void
{
	if(__genModel.masterData.child('jewelry_default').apply_stone_wt_chart.value.toString() == 'Y')
	{
		if(cbStone_shape.dataValue != '' && cbStone_size.dataValue != '' && cbCut.dataValue != '')
		{
			...
			getInformationEvent	= new GetInformationEvent('stonewtinfo', callbacks,'DIAM',cbStone_shape.dataValue,cbStone_size.dataValue,cbCut.dataValue);
