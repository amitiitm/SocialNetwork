...
private function setVisibility(issueType:String):void
{
	if(issueType == 'V' || issueType == 'C')
	{
...
private function setReferenceLookUp(issueType:String):void
{
	...
	switch(issueType.toUpperCase())
	{
		...
		case 'V':
			...
			dcVendCust_id.toolTip		=	'To Vendor'
			dcVendCust_id.dataValue	= '';
			dcVendCust_id.labelValue	= '';
...
{
	if(cbIssue_type.dataValue == 'V' && dcVendCust_id.text != '' && dcVendCust_id.text != null)
	{
	...
	else if(cbIssue_type.dataValue == 'C' && dcVendCust_id.text != "" && dcVendCust_id.text != null)
	{
	...
	else if(cbIssue_type.dataValue == 'S' && dcReference_id.text != "" && dcReference_id.text != null)
	{
...
private function getStoreInfoHandler(resultXml:XML):void
{
	...
	dcReference_id.dataValue	=	resultXml.children().child('company_cd').toString()
...
private function setGrossAmount():void
{
	...
	if(dtlLine.dgDtl.dataProvider != null)
	{
		for(var i:int = 0; i < dtlLine.dgDtl.dataProvider.length; i++)
		{
			...
			_item_qty = _item_qty + (Number)(dtlLine.dgDtl.dataProvider[i].iss_qty);
		}
