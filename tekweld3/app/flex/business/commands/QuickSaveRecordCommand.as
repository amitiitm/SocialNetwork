package business.commands
{
	...
	public class QuickSaveRecordCommand implements ICommand
	{
		...
		protected function saveRecord():int
		{
			...
			//Alert.show('__addEditObj.record'+__addEditObj.record);
			var _id:int = 0 
			if (_id > 0)
			{
		...
		private function saveResultHandler(event:ResultEvent):void
		{
			...
			else
			{
				...
				// we have to fire referesh of LookupRecordSearch if any new record  added
				__genModel.lookupObj.isRecordAdded		=	true;   
				
				if(XML(__addEditObj.record.children()[0]).hasOwnProperty(__genModel.lookupObj.labelField))
				{
					__genModel.lookupObj.labelValue			=	XML(__addEditObj.record.children()[0]).child(__genModel.lookupObj.labelField).toString();	
				}
