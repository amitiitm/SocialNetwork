...
import prod.packagejob.components.PackageJob;
...
[Bindable]
private var __localModel:PackageJobModelLocator =  PackageJobModelLocator(__genModel.activeModelLocator);
...
private function addOptionBar():void
{
	...
	btnGeneratePacakgeSlip.label				= "GENERATE PACAKGE SLIP";
	...
	PackageJob(this.parentDocument).bcp.addChildAt(hb,7);
}
...
private function handleInboxItemFocusOut(event:InboxEvent):void
{
	...
	var oldValue:String = event.oldValues["packaging_flag"].toString();

	if(event.newValues["select_yn"] == 'Y')
	{
		inbox.focusedRow["packaging_flag"] = 'Y';
	}
	else
	{
		inbox.focusedRow["packaging_flag"] = 'N';
	}
