...
import prod.filmjob.FilmJobModelLocator;
import prod.filmjob.components.FilmJob;
...
[Bindable]
private var __localModel:FilmJobModelLocator = (FilmJobModelLocator)(GenModelLocator.getInstance().activeModelLocator);
[Bindable]
...
{	
	...
	//FilmJob(this.parentDocument).bcpDataEntry.addChildAt(btnPrintPickSlip,4);
	FilmJob(this.parentDocument).bcpDataEntry.addChildAt(btnPrintJobDataEntry,5);
	...
	FilmJob(this.parentDocument).bcpList.addChild(refreshLabel);
...
{
	Alert.show("FilmJob"+event.fault.faultDetail);
...
{
	Label(FilmJob(this.parentDocument).bcpList.getChildByName("refreshLabel")).text =	"refresh within "+remainingTime.toString()+" sec."; ;
...
override protected function retrieveRecordEventHandler(event:AddEditEvent):void  //after save and prev,next,.....
{
	...
	if(cbFilm_flag.dataValue=='Y' || cbImpositionReject.dataValue=='Y')
	{
		cbFilm_flag.enabled     	= false;
		cbImpositionReject.enabled  = false;
	}
	else
	{
		cbFilm_flag.enabled     	= true;
...
private function setFlags(event:GenCheckBoxEvent):void
{
	var source:String  = event.target.id.toString();
	if(source	== 'cbFilm_flag')
	{
		if(cbFilm_flag.dataValue=='Y')
		{
			cbImpositionReject.dataValue		= 'N';
			taRejectReason.dataValue			= '';
		}
		else
		{
			cbImpositionReject.dataValue		= 'Y';
			taRejectReason.dataValue			= '';
		}
	}
	else if(source	== 'cbImpositionReject')
	{
		if(cbImpositionReject.dataValue	== 'Y')
		{
			cbFilm_flag.dataValue		= 'N';
		}
		else
		{
			cbFilm_flag.dataValue		= 'Y';
		}
...
override protected function preSaveEventHandler(event:AddEditEvent):int
{
	...
	else if(cbImpositionReject.dataValue=='N')
	{
