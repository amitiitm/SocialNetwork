...
private function getBankInfo():void
{
	var callbacks:IResponder = new mx.rpc.Responder(bankDetailsHandler, null);
	getInformationEvent = new GetInformationEvent('bank_check_info', callbacks, 'receipt');
	getInformationEvent.dispatch();	
}

private function bankDetailsHandler(resultXml:XML):void
{
	tiBank_code.dataValue	=	resultXml.children().child('bank_id').toString();
	dcBank_code.labelValue	=	resultXml.children().child('bank_code').toString();
	dcBank_code.dataValue	=	resultXml.children().child('bank_code').toString();
}

private function handleReceive_amtItemChanged(event:GenTextInputEvent):void
{
