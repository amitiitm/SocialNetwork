...
private var tableNames:Array	=	new Array({tablename:'customer_receipt'});
...
{	
	...
	dgCustomerCreditInvoices.rows	=	resultXml
}

private function clickHandler():void
{
	var message:String	=	XML(dgCustomerCreditInvoices.selectedItem).child('message').toString();
...
override protected function preSaveEventHandler(event:ImportEvent):int
{
	var returnVal:int = 0;
	var str:String;
	var tempList:XMLList = dgCustomerCreditInvoices.rows.children();


	var message:XMLList	=	new XMLList(<message/>);
	
	message	=	dgCustomerCreditInvoices.rows.children().(child('message').toString()	!=	'')
	...
	if(message.children().length() > 0)
	{
		...
		__importObj.records = dgCustomerCreditInvoices.rows;
