...
public function setAppliedAmt():void
{
	...
	var received_amt:Number = Number(tiPaid_amt.text);
...
{
	var receivedAmt:Number = Number(tiPaid_amt.text)
...
private function getBankInfo():void
{
	...
	getInformationEvent = new GetInformationEvent('bank_check_info', callbacks, 'payment');
	getInformationEvent.dispatch();	
}
...
private function handlePayment_amtItemChanged(event:GenTextInputEvent):void
{
	tiApplied_amt.text = '0.00'
	tiBalance_amt.dataValue = (Number(tiPaid_amt.dataValue) - Number(tiApplied_amt.dataValue)).toString()
