...
import ship.directshipping.components.DirectShipping;
import ship.directshipping.components.MultiOptionsShippingPopUp;


[Bindable]
private var __genModel:GenModelLocator = GenModelLocator.getInstance();
[Bindable]
private var __localModel:DirectShippingModelLocator =  DirectShippingModelLocator(__genModel.activeModelLocator);
...
private function handleInboxItemFocusOut(event:InboxEvent):void
{
	...
	if(oldValueshipQty!=newdValueshipQty)
	{
		inbox.focusedRow["multi_description"] = '';
	}
	else
	{
...
override protected function preSaveEventHandler(event:AddEditEvent):int
{
	...
	for(var i:int=0; i<inbox.dgDtl.dataProvider.length; i++)
	{
		...
		{
			var multi_description:String			= inbox.dgDtl.dataProvider[i]["multi_description"];
			var order_multi_description:String		= inbox.dgDtl.dataProvider[i]["order_multi_description"];
			
			if(order_multi_description!='' && multi_description=='')
			{
				Alert.show("Please Specify Ship Qty For Multi Options");
				retValue   = -1;
				break
			}						
		}	
...
private function addOptionBar():void
{
	...
	var btnMultiDescription:Button		= new Button();
	//btnTakeButton.setStyle("icon", imageIcon);
	btnMultiDescription.height				=  20;
	//btnTakeButton.width					=  25;
	btnMultiDescription.toolTip	  			=  "Multi Description";
	btnMultiDescription.label					= "MULTI OPTION QTY";
	btnMultiDescription.styleName				= "promoButton";
	btnMultiDescription.addEventListener(MouseEvent.CLICK,btnMultiDescriptionClickHandler);
	...
	hb.addChild(btnMultiDescription);
	//hb.addChild(btnPrintTruckingSlip);
	
	DirectShipping(this.parentDocument).bcp.addChildAt(hb,7);
}
private function btnMultiDescriptionClickHandler(event:MouseEvent):void
{
	if(inbox.selectedRows.children().length()==1 && inbox.selectedRows.children()[0].order_multi_description.toString()!='')
	{
		var multiOptionsPopUp:MultiOptionsShippingPopUp = MultiOptionsShippingPopUp(PopUpManager.createPopUp(this,MultiOptionsShippingPopUp,true));
		...
		multiOptionsPopUp.inbox = inbox;
	}
	else
	{
		Alert.show("Please Select one Multi Record");
	}
