...
override protected function retrieveRowEventHandler(event:DetailAddEditEvent):void
{
	...
	for(var i:int =0 ; i<rows.children().length(); i++)
	{
		...
		xml.shipping_code					=	rows.children()[i].shipping_code.toString();
		xml.shipping_method					=	rows.children()[i].shipping_method.toString();
		...
		xml.estimated_ship_date				=	rows.children()[i].estimated_ship_date.toString();;
		xml.estimated_arrival_date			=	rows.children()[i].estimated_arrival_date.toString();;
		xml.internal_ship_date				=	rows.children()[i].internal_ship_date.toString();
		xml.internal_ship_date				=	rows.children()[i].internal_ship_date.toString();
		xml.bill_transportation_to			=	rows.children()[i].bill_transportation_to.toString();
		...
		{
			xml.ship_amt					=	numericFormatter2.format(rows.children()[i].ship_amt.toString());
		}		
