...
[Bindable]
private var __localModel:SalesInvoiceModelLocator = (SalesInvoiceModelLocator)(GenModelLocator.getInstance().activeModelLocator);
...
private function setFetchParameter(invoice_type:String, fetch_type:String):void
{
	if(invoice_type == 'O')
	{
		if(fetch_type.toLocaleUpperCase() == 'GRID')
		{
			...
			dtlLine.transactionDetailServiceID="get_so_bom"
...
private function customerDetailsHandler(resultXml:XML):void
{
	...
	{	// Vivek/Rashmi 08 Sep, 2011
		if(dtlLine.dgDtl.rows.children().length() > 0 && (__genModel.masterData['saoi']['other_custs_mem_ord']['value'].toString() != 'Y'))
		{
			for(var i:int=dtlLine.dgDtl.rows.children().length() - 1; i >= 0; i--)
			{
				dtlLine.deleteRow(i)
			}
...
override protected function retrieveRecordEventHandler(event:AddEditEvent):void  //after save and prev,next,.....
{
	...
	var dtlXml:XML = XML(recordXml.children().child('sales_invoice_lines'))
	var _taxableAmount:Number = 0;
...
private function resendInvoice():void
{
	commonLinksServices.resendInvoice();
}
