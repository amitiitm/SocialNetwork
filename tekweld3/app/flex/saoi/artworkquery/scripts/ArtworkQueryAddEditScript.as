...
import saoi.artworkquery.components.ArtworkQuery;
...
[Bindable]
private var __localModel:ArtworkQueryModelLocator = (ArtworkQueryModelLocator)(GenModelLocator.getInstance().activeModelLocator);
[Bindable]
...
private function init():void
{
	...
	ArtworkQuery(this.parentDocument).bcpList.addChild(refreshLabel);
...
{
	Label(ArtworkQuery(this.parentDocument).bcpList.getChildByName("refreshLabel")).text =	"refresh within "+remainingTime.toString()+" sec."; ;
...
{
	Alert.show("RefreshArtworkQuery"+event.fault.faultDetail);
...
private function openIncompleteArtworkScreen():void
{
	var commonMooduleFunctions:CommonMooduleFunctions	= new CommonMooduleFunctions();
	commonMooduleFunctions.drilldownToIncompleteArtworkScreen(tiTrans_no);
}
