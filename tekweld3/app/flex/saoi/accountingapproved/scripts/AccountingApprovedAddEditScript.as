...
import saoi.accountingapproved.components.AccountingApproved;

[Bindable]
private var __genModel:GenModelLocator = GenModelLocator.getInstance();
[Bindable]
private var __localModel:AccountingApprovedModelLocator  =  AccountingApprovedModelLocator(__genModel.activeModelLocator);
...
private function init():void
{
	...
	AccountingApproved(this.parentDocument).bcp.addChild(refreshLabel);
...
{
	Label(AccountingApproved(this.parentDocument).bcp.getChildByName("refreshLabel")).text =	"this screen will automatically refresh within "+remainingTime.toString()+" sec."; ;
...
private function handleInboxItemFocusOut(event:InboxEvent):void
{
		...
		{
			inbox.focusedRow["accountreviewed_flag"] = 'Y';
		}
		else
		{
			inbox.focusedRow["accountreviewed_flag"] = 'N';
		}  
...
{
	Alert.show("AccountingApproved"+event.fault.faultDetail.toString());
