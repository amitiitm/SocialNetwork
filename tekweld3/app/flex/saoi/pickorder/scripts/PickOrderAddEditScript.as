...
import saoi.pickorder.components.PickOrder;

[Bindable]
private var __genModel:GenModelLocator = GenModelLocator.getInstance();
[Bindable]
private var __localModel:PickOrderModelLocator  =  PickOrderModelLocator(__genModel.activeModelLocator);
...
private const pickOrderButtonIcon:Class;
...
private function init():void
{
	...
	PickOrder(this.parentDocument).bcp.addChildAt(hb,7);
	PickOrder(this.parentDocument).bcp.addChild(refreshLabel);
...
{
	Label(PickOrder(this.parentDocument).bcp.getChildByName("refreshLabel")).text =	"refresh within "+remainingTime.toString()+" sec."; ;
...
private function pickOrderHandler(event:MouseEvent):void
{
	...
	if(index>=0)
	{
		...
		var pick:HTTPService = dataService(__service.getHTTPService("pickOrder"));
		__responderPick = new mx.rpc.Responder(pickResultHandler,pickFaultHandler);
		var token:AsyncToken = pick.send(new XML(<params>
												<id>{orderId}</id>
												<orderassignedtouser_id>{__genModel.user.userID}</orderassignedtouser_id>
												<orderassignedtouser_code>{__genModel.user.user_cd}</orderassignedtouser_code>
												</params>));
...
public function pickResultHandler(event:ResultEvent):void
{
	...
	else
	{
		...
		__genModel.objectFromDrilldown	 = drillXml.copy();
		var tabpage:TabPage	=	new TabPage();
