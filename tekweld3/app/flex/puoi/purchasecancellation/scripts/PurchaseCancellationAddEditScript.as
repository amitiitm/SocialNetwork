...
private function setNetAmount():void
{
	...
	if (_gross_amt >= 0)
	{
		...
		tiNet_amt.text = numericFormatter.format(String(_gross_amt  - _dis_amt  + _sal_tax ));
	} 
...
override protected function preSaveEventHandler(event:AddEditEvent):int
{
	...
	if(dtlLine.dgDtl.dataProvider.length > 0)
	{
		for(var i:int = 0; i < dtlLine.dgDtl.dataProvider.length; i++)
		{
			...
			{
				/* Commented as packet validation not required in Item Order Cancellation. 
...
{
	var cancel_type:String = 'O';
	var recordXml:XML		= event.recordXml;
	var dtlXml:XML	=	XML(recordXml.children().child('purchase_order_cancel_lines'))
	var _taxableAmount:Number = 0;
	...
	setFetchParameter(cancel_type, 'GRID')
...
{
	var cancel_type:String = 'O';
	dcStore_id.dataValue	=	GenModelLocator.getInstance().user.default_company_id.toString() //to set default store
...
private function handleBtnGetDataClick(event:Event):void
{
	...
	setFetchParameter(cancel_type, 'BUTTON')
	dtlLine.bcdp.dispatchEvent(new ButtonControlDetailEvent(ButtonControlDetailEvent.FETCH_RECORDS_EVENT));
}

private function setFetchParameter(cancel_type:String, fetch_type:String):void
{
	if(cancel_type == 'O')
	{
		if(fetch_type.toLocaleUpperCase() == 'GRID')
		{
			dtlLine.fetchDetailDataServiceID = "fetch_open_orders";
		...
		{
			dtlLine.fetchDetailDataServiceID = "fetch_open_order_hdr";
