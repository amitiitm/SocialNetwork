...
private var __localModel:PurchaseCreditInvoiceModelLocator = (PurchaseCreditInvoiceModelLocator)(GenModelLocator.getInstance().activeModelLocator);
...
private function setFetchParameter(invoice_type:String, fetch_type:String):void
{
	if(invoice_type == 'I')
	{
		if(fetch_type.toLocaleUpperCase() == 'GRID')
		{
			...
			dtlLine.transactionDetailServiceID="get_purchase_bom"
...
{
	if(dcTerm_code.text != '' && dcTerm_code.text != null)
	{
...
override protected function retrieveRecordEventHandler(event:AddEditEvent):void  //after save and prev,next,.....
{
	...
	var dtlXml:XML = XML(recordXml.children().child('purchase_credit_invoice_lines'))
	var _taxableAmount:Number = 0;
