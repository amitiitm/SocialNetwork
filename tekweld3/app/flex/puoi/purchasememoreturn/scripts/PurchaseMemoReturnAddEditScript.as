...
private function getPacketDetails():void
{
	...
	if(packet_code != '')
	{
		...
		getInformationEvent	= new GetInformationEvent('packetinfovalidate', callbacks, packet_code, item_code, 'Y');
		getInformationEvent.dispatch();
	}
	else
	{
...
private function itemPriceCalculation(rowIndex:int):void
{
	...
	dtlLine.dgDtl.dataProvider[rowIndex].item_qty 		=	numericFormatter.format(dtlLine.dgDtl.dataProvider[rowIndex].item_qty);
...
{
	var return_type:String = 'M';
	var recordXml:XML		= event.recordXml;
	var dtlXml:XML	=	XML(recordXml.children().child('purchase_memo_return_lines'))
	var _taxableAmount:Number = 0;
	...
	setFetchParameter(return_type, 'GRID')
...
{
	var return_type:String = 'M';
	dcStore_id.dataValue = __genModel.user.default_company_id.toString() 
...
private function handleBtnGetDataClick(event:Event):void
{
	...
	setFetchParameter(return_type, 'BUTTON')
	dtlLine.bcdp.dispatchEvent(new ButtonControlDetailEvent(ButtonControlDetailEvent.FETCH_RECORDS_EVENT));
}

private function setFetchParameter(return_type:String, fetch_type:String):void
{
	if(return_type == 'M')
	{
