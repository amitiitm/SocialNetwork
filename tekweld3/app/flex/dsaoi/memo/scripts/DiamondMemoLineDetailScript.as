...
{
	tiDiscount_amt.text	= numericFormatter.format((Number(tiLine_amt.text) * Number(tiDiscount_per.text))/ 100);
}

private function netAmountCalculation():void
{
	tiNet_amt.text = numericFormatter.format(Number(tiLine_amt.text) - Number(tiDiscount_amt.text));
}
...
{
	tiDiscount_per.text	= numericFormatter.format((Number(tiDiscount_amt.text) * 100)/ Number(tiLine_amt.text));
	calculateNetAmount();
}
