module DiamondSale::DiamondInventoryPosting
  include General
  ...
  def check_flags()
    ...
    ri_flag='R'
    case 
    when self.trans_type == 'I' # invoice is issue
      memo_flag = 'N'
      ri_flag = 'T' # for invoice it is transfer, memo receipt/invoice issue.
    when self.trans_type == 'M' # memo is issue
      memo_flag = 'Y'
      ri_flag = 'I'
    when self.trans_type == 'C' # memo is issue
      memo_flag = 'N'
      ri_flag = 'R'
    when self.trans_type == 'R' # memo is issue
      memo_flag = 'Y'
      ri_flag = 'R'
    end 
  ...
  def fill_inventory_detail(ri_flag, memo_flag, diamond_lot_id, diamond_packet_id, iss_pcs, iss_wt, rec_pcs, rec_wt, location_code, line_amt, price, sell_unit,company_id,stone_type,shape,color,clarity,quality)
    ...
    invndetail.customer_vendor_id = self.customer_id
    invndetail.customer_vendor_flag = 'C'
    invndetail.trans_type = 'CUST'
    ...
    if memo_flag =='Y'
      ...
      case 
      when ri_flag == 'T'
        invndetail.memo_rec_pcs = iss_pcs 
        invndetail.memo_rec_wt = iss_wt
        invndetail.memo_rec_amt = line_amt
        invndetail.memo_rec_price = price
